...
static PostGISAdapter* postgisadapter_plg = 0;
...
SPL_PLUGIN_API bool SPL_INIT_NAME_CODE( slcPluginArgs* a_pPluginArgs )
{
	...
	if( a_pPluginArgs->GetArg( 0, arg_ptrs ) == 0 )
	{
		...
		if( params->getVersion() == PLUGINPARAMETERS_VERSION )
		{
			...
			postgisadapter_plg = new PostGISAdapter(params);

			postgisadapter_plg->init();
		}
	}
...
{
	delete postgisadapter_plg;
	postgisadapter_plg = 0;
