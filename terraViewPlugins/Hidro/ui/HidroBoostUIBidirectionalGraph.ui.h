...
* 
* @(#) HidroBoostUIBidirectional Graph.h
*
*******************************************************************************
*
* $Rev: 8870 $:
...
/** 
  * \file HidroBoostUIBidirectional Graph.ui.h
  *
  * \class HidroBoostUIBidirectional Graph
  *
...
/** \brief HidroBoostUIConnectedComponents init function.
    ...
    */
bool HidroBoostUIBidirectionalGraph::init( PluginParameters* parameters, bool bidirectionalGraph)
{
    if(parameters == NULL)
	{
		return false;
	...
	_bidirectionalGraph = bidirectionalGraph;

	if(!_bidirectionalGraph)
	{
		textLabelTitle->setText(tr("Allow execute Search Algorithm over incidence Graphs from BGL..."));
		vertexButtonGroup->setTitle(tr("Init Vertex"));
		this->setCaption(tr("Incidence Graph"));
...
/** \brief HidroBoostUIBidirectional Graph set graph name function.
    ...
    */
void HidroBoostUIBidirectionalGraph::setGraphName( const std::string & graphName )
{
	if(!graphName.empty())
	{
...
/** \brief HidroBoostUIBidirectional Graph Select End Vertex by Click.
    * Used to define the  ended vertex that will be used on boost algorithm
	* Vertex defined by click on canvas app
    */
void HidroBoostUIBidirectionalGraph::endVertexByClickRadioButton_clicked()
{
	_getEndVertex = true;
...
/** \brief HidroBoostUIBidirectional Graph Select End Vertex by identification.
    * Used to define the  Ended vertex that will be used on boost algorithm
	* Vertex defined by identification on interface
    */
void HidroBoostUIBidirectionalGraph::endVertexByIdentRadioButton_clicked()
{
	_getEndVertex = false;
...
void HidroBoostUIBidirectionalGraph::bufferCheckBox_clicked()
{
	if(bufferCheckBox->isChecked())
	{
		bufferDistLineEdit->setEnabled(true);
	}
	else
	{
		bufferDistLineEdit->setEnabled(false);
	}
}

/** \brief HidroBoostUIBidirectional Graph Ok function.
    * Used to execute the operation.
    */
void HidroBoostUIBidirectionalGraph::okPushButton_clicked()
{
	...
	if(bufferCheckBox->isChecked())
	{
		if(bufferDistLineEdit->text().isEmpty())
		{
			QMessageBox::warning(this, tr("Warning"), tr("Buffer distance not defined."));
			return;
		}

		bool correctValue = false;
		bufferDistLineEdit->text().toDouble(&correctValue);
		
		if(!correctValue)
		{
			QMessageBox::warning(this, tr("Warning"), tr("Buffer distance invalid value."));
			return;
		}
	}
...
//execute algorithm
	...
	else
	{
		...
		pBoost._startVertexId		= endVertexIdentLineEdit->text().latin1();
	}
...
//preview terralib objects
		...
		if(res == 0)
		{
			view = persister.createGraphView(&outGraph, proj);
			if(!view)
			{
				QMessageBox::warning(this, tr("Warning"), persister.getErrorMessage().c_str());
				return;
		...
		if(bufferCheckBox->isChecked())
		{
			double value = bufferDistLineEdit->text().toDouble();

			HidroConvexHull cHull(_parameters->getCurrentDatabasePtr());

			TeLayer* layerConvexHull = cHull.generateConvexHull(&outGraph, value);

			//TeLayer* layerConvexHull = cHull.generateMask(&outGraph);

			if(!layerConvexHull)
			{
				QMessageBox::warning(this, tr("Warning"), cHull.errorMessage().c_str());
				return;
			}

			if(res == 0)
			{
				std::string themeName = layerConvexHull->name() + "_theme";
				if(!persister.createTheme(themeName, "", view, layerConvexHull))
				{
					QMessageBox::warning(this, tr("Warning"), persister.getErrorMessage().c_str());
					return;
				}
...
/** \brief HidroBoostUIBidirectional Graph getMouseClicked function.
    ...
    */
void HidroBoostUIBidirectionalGraph::getMouseClicked( TeCoord2D & coord, int /*state*/, QPoint & /*point*/ )
{
