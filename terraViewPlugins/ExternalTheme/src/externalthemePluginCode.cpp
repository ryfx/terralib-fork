...
static ExtThemePlugin* extthemeplugin_plg = 0;
...
SPL_PLUGIN_API bool SPL_INIT_NAME_CODE( slcPluginArgs* a_pPluginArgs )
{
	...
	if( a_pPluginArgs->GetArg( 0, arg_ptrs ) == 0 )
	{
		...
		if( params->getVersion() == PLUGINPARAMETERS_VERSION )
		{
			...
			extthemeplugin_plg = new ExtThemePlugin(params);

			extthemeplugin_plg->init();
		}
	}
...
{
	delete extthemeplugin_plg;
	extthemeplugin_plg = 0;
