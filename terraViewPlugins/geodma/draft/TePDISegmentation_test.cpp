...
void GeoSOMSeg_test()
{
  TePDIParameters params;
  ifstream som_params;
  som_params.open( "som_params.txt" );
  string raster_name, neurons_h, neurons_w, max_epochs, filter_string, min_area, radius, decreasing, alpha, k, fixed_units, tmp;
  som_params >> tmp >> raster_name >> tmp >> neurons_h >> tmp >> neurons_w >> tmp >> max_epochs >> tmp >> filter_string >> tmp >> min_area >> tmp >> radius >> tmp >> decreasing >> tmp >> alpha >> tmp >> k >> tmp >> fixed_units;
  som_params.close();

  cout << "raster: " << raster_name << endl << "neurons_h: " << neurons_h << endl << "neurons_w: " << neurons_w << endl << "max_epochs: " << max_epochs << endl << "filter_string: " << filter_string << endl << "min_area: " << min_area << endl << "radius: " << radius << endl << "decreasing: " << decreasing << endl << "alpha: " << alpha << endl << "k: " << k << endl << "fixed_units: " << (int)(fixed_units == "true") << endl;
  TePDITypes::TePDIRasterPtrType inRaster( new TeRaster(
    std::string( raster_name ), 'r' ) );
  TEAGN_TRUE_OR_THROW( inRaster->init(), "Unable to init inRaster" );
  params.SetParameter( "input_image", inRaster );

  std::vector<unsigned> input_bands;
  for ( unsigned b = 0; b < (unsigned) inRaster->params().nBands(); b++ )
    input_bands.push_back( b );
  params.SetParameter( "input_bands", input_bands );
  ...
  1, 1, 1, false, TeUNSIGNEDLONG, 0 ), "RAM Raster Alloc error" );    
  params.SetParameter( "output_image", outRaster );
  params.SetParameter( "neurons_h", (unsigned) atoi( neurons_h.c_str() ) );
  params.SetParameter( "neurons_w", (unsigned) atoi( neurons_w.c_str() ) );
  params.SetParameter( "max_epochs", atoi( max_epochs.c_str() ) );
  params.SetParameter( "filter_string", filter_string );
  params.SetParameter( "min_area", atoi( min_area.c_str() ) );
  params.SetParameter( "total_coordinates", (unsigned) 2 );
  params.SetParameter( "radius", (float) atof( radius.c_str() ) );
  params.SetParameter( "decreasing", (float) atof( decreasing.c_str() ) );
  params.SetParameter( "alpha", (float) atof( alpha.c_str() ) );
  params.SetParameter( "k", (float) atof( k.c_str() ) );
  params.SetParameter( "fixed_units", (bool) (fixed_units == "true") );

  TePDITypes::TePDIPolSetMapPtrType output_polsets( 
  new TePDITypes::TePDIPolSetMapType );
  params.SetParameter( "output_polsets", output_polsets );

  TePDIGeoSOMSeg segmenter;
  ...
  TEAGN_TRUE_OR_THROW( TePDIUtils::TeRaster2Geotiff( outRaster,
    TEPDIEXAMPLESBINPATH "Segmentation_SOMSeg_test.tif", TeUNSIGNEDCHAR ), 
    "GeoTIF generation error" );   
  ...
  exportPS2SHP( pols, TEPDIEXAMPLESBINPATH "Segmentation_SOMSeg_ps" );
}
