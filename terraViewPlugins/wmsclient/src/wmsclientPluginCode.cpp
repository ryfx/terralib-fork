...
static WMSClientPlugin* wmsclientplugin_plg = 0;
...
SPL_PLUGIN_API bool SPL_INIT_NAME_CODE( slcPluginArgs* a_pPluginArgs )
{
	...
	if( a_pPluginArgs->GetArg( 0, arg_ptrs ) == 0 )
	{
		...
		if( params->getVersion() == PLUGINPARAMETERS_VERSION )
		{
			...
			wmsclientplugin_plg = new WMSClientPlugin(params);

			wmsclientplugin_plg->init();
		}
	}
...
{
	delete wmsclientplugin_plg;
	wmsclientplugin_plg = 0;
