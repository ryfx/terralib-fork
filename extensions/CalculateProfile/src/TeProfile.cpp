...
TeProfile::TeProfile(TeContourLineSet *parLineSet,TeLine2D *parLine)
{
	lineSet = parLineSet;
	lineSight = parLine;
}

TeLine2D TeProfile::calculateProfile()
{
	...
	for(unsigned int j = 0; j < lineSet->size() ; j++)
	{
		for(unsigned int i = 0; i < result[j].size(); i++)
		{
			...
			for(unsigned int k = 0; k < lineSight->size()-1; k++)
			{
				...
				if(TePerpendicularDistance(first, last, pin, TeCoord2D()) < 0.001)
				{
